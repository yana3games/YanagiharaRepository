...
///////////////////////////////////////////////////////////////////////////////
// 全てのシェーダー作成
HRESULT FBXFile::CreateShaders(void)
{
  ...
  if (!shaderManager->LoadVS("./data/SHADER/VertexShader/skinmeshVS.cso", layout, numElements)) return E_FAIL;
  ShaderManager::VertexShader VS = shaderManager->GetVS("./data/SHADER/VertexShader/skinmeshVS.cso");
  m_pVS = VS.pVS;
  m_pIL = VS.pIL;
  if (!shaderManager->LoadGS("./data/SHADER/GeometryShader/skydomeGS.cso")) return E_FAIL;
  m_pGS = shaderManager->GetGS("./data/SHADER/GeometryShader/skydomeGS.cso");
  if (!shaderManager->LoadPS("./data/SHADER/PixelShader/skinmeshPS.cso")) return E_FAIL;
  m_pPS = shaderManager->GetPS("./data/SHADER/PixelShader/skinmeshPS.cso");
}


///////////////////////////////////////////////////////////////////////////////
// 定数バッファの作成
HRESULT FBXFile::CreateConstantBuffer(void)
{ // マトリクスバッファの設定.
  D3D11_BUFFER_DESC bd;
  ZeroMemory(&bd, sizeof(D3D11_BUFFER_DESC));
  bd.ByteWidth = sizeof(FBXStruct::Matrix);
  bd.Usage = D3D11_USAGE_DEFAULT;
  bd.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
  bd.CPUAccessFlags = 0;

  // マトリクスバッファを生成.
  HRESULT hr1 = AppManager::GetRenderer()->GetDevice()->CreateBuffer(&bd, NULL, &m_matCB);
  if (FAILED(hr1)) {
...
void FBXFile::GetFbxModel(void)
{
  ...
  for (int animCnt = 0; animCnt < AnimStackCount; animCnt++)
  {
    ...
    startTime = pTakeInfo->mLocalTimeSpan.GetStart();
    endTime = pTakeInfo->mLocalTimeSpan.GetStop();
    oneFrameTime.SetTime(0, 0, 0, 1, 0, 0, FbxTime::eFrames60);
    allTime[animCnt] = (((int)endTime.Get() - (int)startTime.Get()) / (int)oneFrameTime.Get());
...
//  mesh側でmMaterialIndexDictionary[mesh.mtrName]とやると対応付けされたMaterialが取れる
  ...
  // ノード探査
  pTopMyNode = MY_NODE::recursiveNode(pManager, pScene->GetRootNode(), m_bTexcoordVReverse, this);
...
///////////////////////////////////////////////////////////////////////////////
// マテリアルの取得（どのメッシュに対応？）
void MY_NODE::analyzeMaterial(FbxMesh* mesh)
{
...
///////////////////////////////////////////////////////////////////////////////
// マテリアルの取得（ノードから）
void MY_NODE::analyzeMaterial(FbxNode* node)
{
  // マテリアル数の取得
...
//        std::string strPathName = "data\\MODEL\\dude\\" + extname;
        std::string strPathName = "data\\MODEL\\" + extname;
        ...
        // テクスチャの読み込みと管理
       // AppManager::GetTextureManager()->Load(wc);
...
///////////////////////////////////////////////////////////////////////////////
// クラスター（ボーン）の取得
void analyzeCluster(FbxMesh* mesh)
{
  // スキンの数を取得
  ...
  for (int i = 0; i < skinCount; ++i) {
    ...
    for (int clusterNum = 0; clusterNum < clusterCount; ++clusterNum){
      ...
      matGeometry.SetT(mesh->GetNode()->GetGeometricTranslation(FbxNode::eSourcePivot));
      matGeometry.SetR(mesh->GetNode()->GetGeometricRotation(FbxNode::eSourcePivot));
      matGeometry.SetS(mesh->GetNode()->GetGeometricScaling(FbxNode::eSourcePivot));

      lReferenceGlobalInitPosition *= matGeometry;
      ...
      for (int i = 0; i < pointIndexCount; i++) {
        // 頂点インデックスとウェイトを取得
        int pointNum = pointIndexArray[i];
      }
    }
  }
}
...
///////////////////////////////////////////////////////////////////////////////
// アトリビュート名を頂く
std::string MY_NODE::GetAttributeTypeName(FbxNodeAttribute::EType type)
{
  switch (type) {
...
///////////////////////////////////////////////////////////////////////////////
// 頂点座標の取得
void MY_NODE::analyzePosition(FbxMesh* mesh)
{
  // コントロールポイント数の取得
...
///////////////////////////////////////////////////////////////////////////////
// 頂点バッファ作成
void MESH_FBX::CreateVertexBuffer()
{
  HRESULT result;
...
///////////////////////////////////////////////////////////////////////////////
// ノードを掘る
MY_NODE* MY_NODE::recursiveNode(FbxManager* pManager, FbxNode* pNode, bool bTexcoordVReverse, FBXFile* uniqueKun)
{
  MY_NODE* p = NULL;
  ...
  if (pNode) {
    ...
    mat = DirectX::XMMatrixTranslation(p->translation.x, p->translation.y, p->translation.z) * DirectX::XMMatrixRotationRollPitchYaw(p->rotation.z, p->rotation.x, p->rotation.y) * DirectX::XMMatrixScaling(p->scaling.x, p->scaling.y, p->scaling.z);
...
///////////////////////////////////////////////////////////////////////////////
// 法線取得
void MY_NODE::analyzeNormal(FbxMesh* pMesh)
{
  // レイヤー数の取得
...
///////////////////////////////////////////////////////////////////////////////
// テクスチャ座標取得
void MY_NODE::analyzeTexcoord(FbxMesh* pMesh, bool bRevers)
{
  int layerCount = pMesh->GetLayerCount();
...
///////////////////////////////////////////////////////////////////////////////
// クラスター（ボーン）の取得
void MY_NODE::analyzeCluster(FbxMesh* mesh, FBXFile* uniqueKun)
{
  // スキンの数を取得
  int skinCount = mesh->GetDeformerCount(FbxDeformer::eSkin);
...
///////////////////////////////////////////////////////////////////////////////
// ジオメトリーのオフセット取るよー
FbxAMatrix MY_NODE::GetGeometry(FbxNode* node)
{
...
///////////////////////////////////////////////////////////////////////////////
// ノード順に描画処理を実行
void MY_NODE::recursiveDraw(const DirectX::XMMATRIX& mtxWorldParent, ID3D11Buffer* matCB, ID3D11Buffer* boneCB)
{
  // シェーダを設定して描画
  ...
  DirectX::XMMATRIX mtxRotate = DirectX::XMMatrixRotationRollPitchYaw( rotation.z, rotation.x, rotation.y);
  ...
  if (!meshes.empty()) {
    for (auto itm = meshes.begin(); itm != meshes.end(); ++itm) {
      ...
      FBXStruct::Matrix cb;
      cb.world = mtxWorld;
      cb.view = *CCameraMan::GetInstance()->GetViewMtx();
      cb.proj = *CCameraMan::GetInstance()->GetProjMtx();

      // サブリソースを更新.
      pDeviceContext->UpdateSubresource(matCB, 0, NULL, &cb, 0, 0);

      // 頂点シェーダに定数バッファを設定.
      pDeviceContext->VSSetConstantBuffers(0, 1, &matCB);
    }

      //DirectX::XMMATRIX mtxView = *CCameraMan::GetInstance()->GetViewMtx();
      //DirectX::XMMATRIX mtxProj = *CCameraMan::GetInstance()->GetProjMtx();

      //Shader* shader = AppManager::GetShaderManager()->GetShader(SHADEROBJECT_SKIN);
      //shader->SetParameters(boneCB, itm->vertexBuffer, mtxWorld, mtxView, mtxProj, mtxWIT, texture);
      // プリミティブの種類を設定
  ...
  std::for_each(children.begin(), children.end(), [&](MY_NODE* p) { p->recursiveDraw(mtxWorld, matCB, boneCB); });
}


///////////////////////////////////////////////////////////////////////////////
// 描画
void FBXFile::Draw(void) {
// 定数バッファの設定.
//  DirectX::XMMATRIX bones[ 512];
//  for (int numBone = 0; numBone < m_bones.size(); numBone++) {
//    bones[numBone] = m_bones[numBone][currentFrame%m_bones[0].size()];
//  }
...
//  pTopMyNode->recursiveDraw(m_world, m_matCB, m_boneCB);
  ...
  // 頂点シェーダに定数バッファを設定.
  pDeviceContext->VSSetConstantBuffers(1, 1, &m_boneCB);
